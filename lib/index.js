...
Launch.command("playstore", "Deploy to Google Play Store").option('-a, --amount <channel>', 'upload channel.').action(function (opt) {
  _android2.default.prepareApk(superEnv, opt.options.amount).then(function () {
    return _play2.default.uploadPlayStore(superEnv, opt.options.amount);
  }).catch(function (error) {
    return console.log(error.message);
  });
});
